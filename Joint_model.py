...
def inputs_to_tree_reps(head, words, l):
    trees = [head_to_tree(head[i], words[i], l[i].int()) for i in range(len(l))]
    maxlen = l.max().int().item()
    adj = [tree_to_adj(maxlen, tree).reshape(1, maxlen, maxlen) for tree in trees]
...
class JointClassifier(nn.Module):
    def __init__(self, args, emb_matrix=None):
        ...
        # GCN
        if args.use_GCN == True:
            self.W = nn.ModuleList()
            for layer in range(args.layers_GCN):
                self.W.append(nn.Linear(2*args.rnn_hidden, 2*args.rnn_hidden))
    ...
    # 0: tokens, 1: pos, 2: mask_s, 3: head
    def forward(self, inputs):

        tokens, pos, mask_s, head = inputs
        ...
        # GCN
        if self.args.use_GCN == True:
            # trans head to adj
            adj = inputs_to_tree_reps(head.data, tokens.data, lens.data)
            # GCN forward
            denom = adj.sum(2).unsqueeze(2) + 1    # norm
            for l in range(self.args.layers_GCN):
                Ax = adj.bmm(H)
                AxW = self.W[l](Ax)
                AxW = AxW / denom
                H = F.relu(AxW)
