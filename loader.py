...
class DataLoader(object):
    ...
    def preprocess(self, data, dicts, args):
        ...
        for d in data:
            ...
            for en in d['en_list']:
                # correct the error produced by parsing the en_list
                if en[-1] == '.':
                    en = en[0:-1]
                    en[-1] = en[-1]+'.'
            ...
            processed += [(tokens, pos, mask_s, (d['sentText'], d['relationMentions']), NER_labels, head)]
        return processed
    ...
    def __getitem__(self, key):
        ...
        head = get_long_tensor(batch[5], batch_size)

        return (words, pos, mask_s, RC_labels, NER_labels, head)
